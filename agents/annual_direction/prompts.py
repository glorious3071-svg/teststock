...
## 申万行业信号使用方法（`sw_sector_signals`）
数据包中含有申万一级行业的量化信号，用于辅助筛选行业赛道：
- `momentum_1y_pct`: 过去1年涨跌幅。负值代表大跌（均值回归机会），高于30%代表强势（趋势追随）。
- `pb_pct_5y`: 当前PB在近5年历史中的百分位。<25% 估值偏低，>75% 估值偏高。
- `signal`: 综合信号标签（参考用，不要机械套用）。

结合政策定调与宏观背景，筛选符合以下条件之一的行业赛道：
  A. 政策重点支持 + 估值合理或偏低 → 确定性较强
  B. 大幅回调 + 政策预期改善 → 困境反转机会
  C. 强势行业 + 基本面持续改善 → 趋势延续

## 输出要求
每次回复须包含：
1. **宏观判断**（2-4 段）
2. **行业赛道筛选**：基于 CEWC 关键词和 sw_sector_signals，列出 3-5 个候选方向及理由
3. **配置建议**（JSON 代码块，严格格式如下）
...
数据包中的 `sw_sector_signals` 包含申万一级行业的量化信号（动量 + PB 百分位），请结合 CEWC 政策关键词和宏观背景，重点分析：
1. 政策重点支持的行业（提炼 CEWC 关键词，如「绿色发展→新能源」「科技自立→半导体」）
2. 估值和动量信号强化或弱化以上判断的行业
3. 给出最终行业赛道选择逻辑，再从候选 ETF 池中匹配标的
