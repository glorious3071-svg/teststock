...
"""backtest_scorecard_national_team.py — v3.4.9 候选：国家队入场接入评分卡 3 组合回测

对比：
  - baseline    : national_team_action=None
  - +normal     : 含年度例行增持（intensity ≥ normal）
  - +strong     : 仅救市强信号（intensity = strong）

P&L 模型与 v3.4.5-v3.4.8 一致。采纳判据：累计回报 ↑ + 最大回撤 Δ≤+5pp + Spearman ρ ≤ baseline。
"""
...
OUT_PATH = ROOT / "data" / "backtests" / "scorecard_national_team_comparison.json"

CANDIDATES = {
    "baseline": AdapterOptions(include_national_team=False),
    "+normal":  AdapterOptions(include_national_team=True,
                                national_team_min_intensity="normal"),
    "+strong":  AdapterOptions(include_national_team=True,
                                national_team_min_intensity="strong"),
}
...
def run() -> dict:
            ...
            for year in range(START_YEAR, END_YEAR + 1):
                ...
                for name, opts in CANDIDATES.items():
                    ...
                    yearly[year]["evals"][name] = {
                        ...
                        "pnl_pct": pnl,
                        "national_team": inp.national_team_action,
                    }
    finally:
        conn.close()
    ...
    for n in CANDIDATES:
        header += f"{n+' tone':>14}{n+' 分':>9}{n+' 仓':>7}"
    print(header)
    print("-" * len(header))
    for y in years:
        ...
        for n in CANDIDATES:
            ev = yearly[y]["evals"][n]
            tone = str(ev.get("national_team") or "")[:9]
            row += f"{tone:>14}{ev['score']:>+9d}{ev['target_equity_pct']:>6.0f}%"
        print(row)
