...
"""backtest_scorecard_meeting_tone.py — v3.4.8 候选：中央会议口径接入评分卡 3 组合回测

对比：
  - baseline    : central_meeting_tone=None（当前状态，policy 规则从未触发）
  - +annual     : 仅 cewc_annual.tone+theme 关键字判别
  - +tags       : cewc_tags（key_phrase+primary_focus）+ cewc_annual 复合判别

P&L 模型与 v3.4.5/v3.4.6 一致。采纳判据：累计回报 ↑ + 最大回撤 Δ≤+5pp + Spearman ρ ≤ baseline。
"""
...
OUT_PATH = ROOT / "data" / "backtests" / "scorecard_meeting_tone_comparison.json"

CANDIDATES = {
    "baseline": AdapterOptions(include_central_meeting_tone=False),
    "+annual":  AdapterOptions(include_central_meeting_tone=True,
                                central_meeting_tone_source="annual_only"),
    "+tags":    AdapterOptions(include_central_meeting_tone=True,
                                central_meeting_tone_source="tags_first"),
}
...
def compute_metrics(scores: list[int], rets: list[float], pnls: list[float]) -> StrategyMetrics:
...
def run() -> dict:
            ...
            for year in range(START_YEAR, END_YEAR + 1):
                ...
                for name, opts in CANDIDATES.items():
                    ...
                    yearly[year]["evals"][name] = {
                        ...
                        "pnl_pct": pnl,
                        "meeting_tone": inp.central_meeting_tone,
                    }
    finally:
        conn.close()
    ...
    for y in years:
        ...
        for n in CANDIDATES:
            ev = yearly[y]["evals"][n]
            tone = str(ev.get("meeting_tone") or "")[:11]
