...
"""backtest_scorecard_pboc_tone.py — v3.4.4 候选：pboc_tone 接入评分卡 3 组合回测

对比：
  - baseline    : pboc_tone=None（当前状态，policy 维度的央行口径规则从未触发）
  - +annual     : 从 cewc_annual.monetary_policy 读 pboc_tone
  - +tags       : cewc_tags 优先 + cewc_annual 兜底（推荐路径）

策略 / P&L 模型与之前回测一致：按 target_equity 配沪深300 + 现金（2% 年化）。
评价指标：累计回报 / 年化收益 / 年化波动 / 最大回撤 / Spearman ρ / 方向命中率。
采纳判据：累计回报 ↑ + 最大回撤 Δ≤+5pp + Spearman ρ ≤ baseline（即更负即更准）。
"""
...
OUT_PATH = ROOT / "data" / "backtests" / "scorecard_pboc_tone_comparison.json"

CANDIDATES = {
    "baseline": AdapterOptions(include_pboc_tone=False),
    "+annual":  AdapterOptions(include_pboc_tone=True, pboc_tone_source="annual_only"),
    "+tags":    AdapterOptions(include_pboc_tone=True, pboc_tone_source="tags_first"),
}
...
def run() -> dict:
    ...
    for n in CANDIDATES:
        header += f"{n+' tone':>12}{n+' 分':>9}{n+' 仓':>7}"
    print(header)
    print("-" * len(header))
    for y in years:
        ...
        for n in CANDIDATES:
            ev = yearly[y]["evals"][n]
            tone = str(ev.get("pboc_tone") or "")[:7]
            row += f"{tone:>12}{ev['score']:>+9d}{ev['target_equity_pct']:>6.0f}%"
        print(row)
    ...
    print(f"\n=== 指标汇总 ===")
    print(f"{'指标':<28}" + "".join(f"{n:>12}" for n in CANDIDATES))
    print("-" * (28 + 12 * len(CANDIDATES)))
    for label, key in [
        ...
        row = f"{label:<28}"
        for n in CANDIDATES:
            row += f"{getattr(metrics[n], key):>12.2f}"
        print(row)

    print(f"\n=== 采纳判定（每个 candidate vs baseline）===")
    decisions = {}
    for n in ["+annual", "+tags"]:
        m = metrics[n]
        criteria = {
