...
| **非制造业PMI>55（过热）** (v10-R1) | `pmi_non_mfg > 55` | risk | +1 |
| **非制造业PMI<50（收缩→反弹）** (v10-R1) | `pmi_non_mfg < 50` | opportunity | -1 |
...
>
> **v10-R1 新增（2026-06）**：非制造业 PMI 作为独立信号。基于 18 年回测诊断发现 `pmi_non_mfg` 的 ρ_12m=-0.325（强反向），是评分卡之前的最大遗珠。新增 >55 过热 +1 / <50 收缩反弹 -1。simulate 严格红线 3/3 通过：累计 P&L +3.15 pp、回撤改善 +0.99 pp、Spearman ρ_12m 从 -0.086 → **-0.122**（提升 41%）。
>
...
| **黄金 YoY>25%（全球放水）**（v11-G）| `gold_yoy_pct > 25` | opportunity | -1 |
| **VIX 月均>30（恐慌底）**（v11-V）| `vix_30d_avg > 30` | opportunity | -1 |
| **FED 利率≥4.5（紧缩末段）**（v11-F）| `fed_rate_level >= 4.5` | opportunity | -1 |
...
>
> **v11 GVF 三合一外部宏观补强（2026-06）**：基于「国际形势复杂化对 A 股影响越来越大」的现实需求，单变量分析 9 个外部信号后采纳 3 个反直觉强信号：
>  - **gold_yoy_pct > 25%** ⭐ ρ_12m=+0.513（**全样本最强外部信号**），89% 命中 17/19，逻辑：黄金大涨=全球流动性放水周期=A 股共振
>  - **vix_30d_avg > 30** ρ_12m 触发后均涨 +29%，80% 命中 16/20，逻辑：极端恐慌底
>  - **fed_rate_level ≥ 4.5** ρ_12m=+0.201，71% 涨，反直觉：紧缩末段=A 股提前定价宽松预期
>
> simulate 严格红线 3/3 全通过：v10 baseline ρ_12m=-0.122 → v11-GVF -0.172（改善 41%）、累计 P&L +0.61 pp、回撤持平。
